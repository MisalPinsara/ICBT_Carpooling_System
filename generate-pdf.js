...
// ── Colors ───────────────────────────────────────────────────────────────────
const COLOR_HEADER_BG = "#1e40af"; // Deep Royal Blue
const COLOR_HEADER_TEXT = "#ffffff";
const COLOR_ROW_ALT = "#f8fafc";
const COLOR_ROW_EVEN = "#ffffff";
const COLOR_BORDER = "#cbd5e1";
const COLOR_TEXT = "#1e293b";
const COLOR_STATUS_PASS = "#15803d"; // Emerald green for Passed

// ── Column Definitions ───────────────────────────────────────────────────────
const columns = [
  { name: "Test ID", width: 50, align: "left" },
  { name: "Related User Story", width: 75, align: "left" },
  { name: "Feature/Module", width: 68, align: "left" },
  { name: "Test Scenario", width: 110, align: "left" },
  { name: "Test Data", width: 85, align: "left" },
  { name: "Test Steps / Test Script", width: 105, align: "left" },
  { name: "Expected Result", width: 105, align: "left" },
  { name: "Actual Result", width: 70, align: "left" },
  { name: "Status", width: 44, align: "center" },
  { name: "Evidence Reference", width: 75, align: "left" }
];

const startX = 25;
const pageWidth = 841.89 - 50; // 791.89 pt printable area
...
// Helper to draw text within bounded rectangle with auto-wrap
function drawWrappedCell(text, x, y, width, height, fontName = "Helvetica", fontSize = 7, align = "left", textColor = COLOR_TEXT) {
...
// Calculate height needed for a row
...
function drawHeader(y) {
  // Title & subtitle above header if on first page
  let currentX = startX;

  // Header background bar
  doc.rect(startX, y, pageWidth, 22).fill(COLOR_HEADER_BG);

  columns.forEach((col) => {
    doc.font("Helvetica-Bold").fontSize(7.5).fillColor(COLOR_HEADER_TEXT);
    doc.text(col.name, currentX + 3, y + 6, {
      width: col.width - 6,
      align: col.align
    });
    currentX += col.width;
...
// Header / Title block on first page
...
doc.text(
  "Evidence reference for all rows: terminal screenshot showing npm test completed successfully with 5 passed test suites and 71 passed tests.",
  startX,
  currentY,
  { align: "left", width: pageWidth }
);
...
testCases.forEach((row, idx) => {
  ...
  // Check if row fits on current page (page height = 595.28, margin = 25 -> bottom limit = 570)
  ...
  columns.forEach((col) => {
    ...
    drawWrappedCell(text, cellX, currentY, col.width, rowHeight, fontName, 7, col.align, textColor);
    cellX += col.width;
  });
